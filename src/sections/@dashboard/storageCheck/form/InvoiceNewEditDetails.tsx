  ...
  const handleSelectService = useCallback(
    (index: number, option: string) => {
      setValue(
        ...
        signboard?.find((service) => service.id === option)?.signboard.name
      );
    },
            ...
            <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} sx={{ width: 1 }}>
              ...
              >
                ...
                {signboard?.map((option) => (
                  <MenuItem
                    key={option.signboard.id}
                    value={option.signboard.id}
                  ...
                  >
                    <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                      <Avatar alt={option.signboard.name} src={option.signboard.image} />
                      {option.signboard.name}
                    </Box>
                  </MenuItem>
                ))}
              </RHFSelect>
