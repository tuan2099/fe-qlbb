...
import { addTransfer, updateTransfer } from 'src/apis/transfer.api';
// ----------------------------------------------------------------------
...
export default function InvoiceNewEditForm({ isEdit, currentInvoice }: Props) {
  ...
  const NewUserSchema = Yup.object().shape({
    from_storage_id: Yup.mixed().nullable().required('Chọn kho chuyển'),
    to_storage_id: Yup.mixed().nullable().required('Chọn kho đến'),
  });

  const defaultValues = useMemo(
    () => ({
      from_storage_id: currentInvoice?.from_storage_id || null,
      to_storage_id: currentInvoice?.to_storage_id || null,
  ...
  const handleCreate = useMutation({
    mutationFn: (data: any) => addTransfer(data),
    onSuccess: () => {
  ...
  const handleUpdate = useMutation({
    mutationFn: (data: any) => {
      const cleanedData = {
        ...data,
        from_storage_id:
          typeof data.from_storage_id === 'object' ? data.from_storage_id.id : data.from_storage_id,
        to_storage_id:
          typeof data.to_storage_id === 'object' ? data.to_storage_id.id : data.to_storage_id,
      };
      return updateTransfer({ id, data: cleanedData });
    },
    onSuccess: () => {
