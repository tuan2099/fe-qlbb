...
import { deleteTransfer, getAllTransfer } from 'src/apis/transfer.api';
...
const TABLE_HEAD = [
  ...
  { id: 'from_storage', label: 'Kho chuyển', align: 'left' },
  { id: 'to_storage', label: 'Kho đích', align: 'left' },
...
const WarehouseDispatchPage = () => {
  ...
  const {
    ...
    queryKey: ['transfer', page2],
    queryFn: () => getAllTransfer({ page: page2 }),
  });
  ...
  const handleEditRow = (id: number) => {
    navigate(`/dashboard/warehouse-transfer/update/${id}`);
  };
      ...
      <Container maxWidth={themeStretch ? false : 'xl'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Chuyển kho', href: PATH_DASHBOARD.user.root },
            { name: 'Danh phiếu chuyển' },
          ]}
          action={
            <>
              {hasPermission('transfer_create') && (
                <Button
                  to="/dashboard/warehouse-transfer/add"
